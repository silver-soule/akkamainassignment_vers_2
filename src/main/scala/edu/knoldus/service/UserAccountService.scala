...
import edu.knoldus.UserAccountGenerator.BillerLinkedStatus
...
class UserAccountService {
  ...
  def createAccounts(accounts: List[List[String]], accountGeneratorRef: ActorRef): Future[List[Boolean]] = {
    implicit val timeout = Timeout(10 seconds)

    val createdAccounts =
      for {
        account <- accounts
        accountnumToBool = (accountGeneratorRef ? account).mapTo[Boolean]
